...
| **docs/**                                   | Documentation folder. Includes:<br>• `images_design_diagrams/` – all design diagrams of the JPEG encoder.<br>• `images_testbench_EO_CO/` – original output vs expected output test images.<br>• `index.md` – detailed project documentation.                                                                                                                                                            |
| **output\_images/**                         | Stores all reconstructed JPEG images.                                                                                                                                                                                                                                                 
| **rtl/**                                    |Contains all SystemVerilog RTL modules for the JPEG encoder. The pipeline is:
- Color Conversion: RGB → Y, Cb, Cr
- DCT: Y-DCT, Cr-DCT, Cb-DCT
- Quantizers: Y-Quantizer, Cr-Quantizer, Cb-Quantizer
- Huffman Encoding: Y-Huff, Cr-Huff, Cb-Huff
- Bitstream Handling: 4 modules for final JPEG bitstream assembly                                                                                                                                                                                                                                                                              |
| **raw\_\_jpeg\_\_bitstream\_\_to\_\_image/** | Handles raw bitstream post-processing to generate final compressed images. Includes:<br>• `bitstream__output.txt` – output file generated after running QuestaSim simulation (raw JPEG bitstream in hex).<br>• `header.bin` – predefined JPEG header (resolution, quantization tables, Huffman tables, etc.).<br>• `jpeg.py` – combines the header + bitstream and generates the compressed JPEG image. |
| **testimages/**                             | Sample test images used for verifying the encoder functionality.                                                                                                                                                                                                                                                                                                                                        |
| **scripts/**                                | Contains helper scripts and I/O images. Includes:<br>• `input_image/` – stores original uncompressed images.<br>• `data_in.py` – converts input images into 24-bit BGR SystemVerilog testbench format (`data_in <= 24'bBBBBBBBBGGGGGGGGRRRRRRRR;`).<br>• `compressed_image/` – stores compressed images generated after full pipeline.                                                                  |
| **testbenches/**                            | Verification testbenches for modules and full encoder. Includes:<br>• `rgb2ycbcr_tb/` – testbench for RGB → YCbCr conversion.<br>• `dct_tb/` – testbench for 8×8 Discrete Cosine Transform.<br>• `quantization_tb/` – testbench for quantization.<br>• `huffman_tb/` – testbench for Huffman encoding.<br>• `jpeg_top_TB/` – top-level testbench for full JPEG encoder simulation.                      |
